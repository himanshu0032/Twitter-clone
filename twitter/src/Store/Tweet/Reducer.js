...
export const tweetReducer= (state = initialState, action)=>{
   switch (action.type) {
    ...
    case TWEET_CREATE_SUCCESS:
        return {
            ...state,loading:false,error:null, tweets:[action.payload, ...state.tweets]
        };
    
    case GET_ALL_TWEETS_SUCCESS:
        case  GET_USERS_TWEETS_SUCCESS:
            return {
                ...state,loading:false,error:null, tweets: action.payload
            };
    
    case USER_LIKE_TWEET_SUCCESS:
            return {
                ...state,loading:false,error:null, likedTweets: action.payload
            }; 
    
    case LIKE_TWEET_SUCCESS:
        return {
            ...state,loading:false,error:null, like: action.payload
        };
    
    case TWEET_DELETE_SUCCESS:
        return{
            ...state,loading:false,error:null, tweets: state.tweets.filter((tweet)=> tweet.id!==action.payload)
        }
    
    case RETWEET_SUCCESS:
        return {
            ...state,loading:false,error:null, retweet: action.payload
        };

    case FIND_TWEET_BY_ID_SUCCESS:
        case REPLY_TWEET_SUCCESS:
        return {
            ...state,loading:false,error:null, tweet: action.payload
        };     
